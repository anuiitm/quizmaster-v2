  ...
  <div class="container mt-5">
    ...
    <form @submit.prevent="handleSubmit" class="card p-4 shadow-sm" style="max-width: 600px; margin: auto;">
      ...
      <div class="d-flex justify-content-center align-items-center gap-3 mt-3">
        ...
        <button type="submit" class="btn btn-success btn-lg action-btn d-flex align-items-center justify-content-center">Save Changes</button>
      </div>
    </form>
  </div>
</template>
...
const fetchChapter = async () => {
  try {
    const res = await apiClient.get(`/api/admin/chapter/${chapterId}`);
    form.value = {
      name: res.data.name,
      description: res.data.description
    };
  } catch (err) {
    errors.value.global = 'Failed to fetch chapter.';
  }
};
...
const handleSubmit = async () => {
  ...
  message.value = '';
  try {
    await apiClient.put(`/api/admin/chapter/${chapterId}`, form.value);
